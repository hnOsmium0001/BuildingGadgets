...
class RegionSpliterator implements Spliterator<BlockPos> {
    ...
    private int posY;
    private int posZ;
    private boolean allowYZSplit; //as soon as this spliterator advanced once, it can no longer be guaranteed, that the given blocks have not yet been visited
    ...
    /**
     *
     * @return A part of this spliterator, if splitting is possible
     */
    @Override
    public Spliterator<BlockPos> trySplit() {
        ...
        int oldMinY = minY;
        int oldMinZ = minZ;
        //construct new min coordinates, so that at least one can be splitted of - max-min>=1
